...
int
app_l2_learning(const struct ether_addr* srcaddr, uint8_t port, uint32_t port_id) {
	if (app.l2_hash == NULL) {
		RTE_LOG(
			ERR, HASH,
	...
	if (index == -EINVAL) {
		...
		RTE_LOG(
			INFO, HASH,
			"%s: new item in forwarding table in core for port[%u]:"
		...
		if (old_port != port) {
			RTE_LOG(
				INFO, HASH,
				"%s: Update item in forwarding table in core for port[%u]:"
				" %02" PRIx8 " %02" PRIx8 " %02" PRIx8
				" %02" PRIx8 " %02" PRIx8 " %02" PRIx8
				"--> %d (previous: %d)\n",
				__func__, port_id,
...
int
app_l2_lookup(const struct ether_addr* addr, uint32_t port_id) {
	...
	/*	if (interval <= app.fwd_item_valid_time) {
			...
			RTE_LOG(
				INFO, HASH,
				"%s: Fowllowing item is outdated, delete it from forwarding table in core for port[%u]:"
				" %02" PRIx8 " %02" PRIx8 " %02" PRIx8
				" %02" PRIx8 " %02" PRIx8 " %02" PRIx8
				"--> %d\n",
				__func__, port_id,
...
void app_main_loop_forwarding_each_port(uint32_t port_id){
	...
	while(!force_quit){
		...
		app_l2_learning(&(eth->s_addr), port_id, port_id);
...
app_main_loop_forwarding(void) {
	...
	for (i = 0; !force_quit; i = ((i + 1) & (app.n_ports - 1))) {
		...
		// l2 learning
		eth = rte_pktmbuf_mtod(worker_mbuf->array[0], struct ether_hdr*);
		app_l2_learning(&(eth->s_addr), i, i);

		// l2 forward
		dst_port = app_l2_lookup(&(eth->d_addr), i);
		if (dst_port < 0) { /* broadcast */
			RTE_LOG(DEBUG, SWITCH, "%s: broadcast packets\n", __func__);
			for (j = 0; j < app.n_ports; j++) {
				if (j == i) {

...
app_init_rings(void) {
    ...
    app.ring_long_size = (topower2(app.buff_size_bytes / MEAN_PKT_SIZE) << 2);
    for (i = 0; i < app.n_ports; i++) {
        char name[32];

        snprintf(name, sizeof(name), "app_ring_long_%u", i);

        app.rings_long[i] = rte_ring_create(
            name,
            app.ring_long_size,
            rte_socket_id(),
            RING_F_SP_ENQ | RING_F_SC_DEQ);

        if (app.rings_long[i] == NULL)
